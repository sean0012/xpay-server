...
router.get('/trade_hist',
	...
	async (req, res) => {
		...
		const data = transfers.map(transfer => ({
			...
			amount: transfer.amount ? transfer.amount.toString() : undefined,
			...
			fee: transfer.fee ? transfer.fee.toString() : undefined,
			//dynamic_code: transfer.dynamic_code,
			//expiry: new Date(transfer.expiry).getTime(),
			type: transfer.type,
			status: transfer.status,
			approval_id: transfer.approval_id,
			payer_points_using: transfer.payer_points_using ? transfer.payer_points_using.toString() : undefined,
			payer_points_gained: transfer.payer_points_gained ? transfer.payer_points_gained.toString() : undefined,
			memo: transfer.memo,
